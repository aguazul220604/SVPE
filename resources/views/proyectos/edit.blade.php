...
<head>
    ...
    <link rel="stylesheet" href="{{ asset('css/proyectos/edit.css') }}">
</head>
    ...
    @section('content')
        <div class="section-card">
            ...
            <form method="POST" action="{{ route('proyectos.update', $proyecto->idProyecto) }}"
                enctype="multipart/form-data" class="formulario-principal">
                @csrf
                @method('PUT')
                <div class="section">
                        ...
                        <label>
                            ...
                            <select name="IdUsuarioLider" required>
                                ...
                                @foreach ($lideres as $lider)
                                    <option value="{{ $lider->idUsuario }}"
                                        @if (isset($liderAsignado) && $liderAsignado->idUsuario == $lider->idUsuario) selected @endif>
                                        {{ $lider->nombres }} {{ $lider->apellido_paterno }} ({{ $lider->matricula }})
                                    </option>
                                @endforeach
                            </select>
                        </label>
                            ...
                            <select name="IdAsesor">
                                ...
                                @foreach ($asesores as $asesor)
                                    <option value="{{ $asesor->idUsuario }}"
                                        @if (isset($asesorAsignado) && $asesorAsignado->idUsuario == $asesor->idUsuario) selected @endif>
                                        {{ $asesor->nombres }} {{ $asesor->apellido_paterno }}
                                    </option>
                                @endforeach
                            </select>
                        </label>
                            ...
                            <select name="IdCategoria" id="IdCategoria" required>
                                ...
                                @foreach ($categorias as $categoria)
                                    ...
                                    <option value="{{ $categoria->idCategoria }}"
                                        {{ optional($proyecto->convocatoria->pertenencia->categoria)->idCategoria == $categoria->idCategoria ? 'selected' : '' }}>
                        ...
                        <label>
                            ...
                            <select name="IdConvocatoria" id="IdConvocatoria" required>
                                ...
                                @if ($proyecto->convocatoria)
                                    <option value="{{ $proyecto->convocatoria->idConvocatoria }}" selected>
                            ...
                            <select name="IdEstado" required>
                                ...
                                @foreach ($estatus as $status)
                                    <option value="{{ $status->idEstado }}"
                                        {{ $proyecto->descripcion->idEstado == $status->idEstado ? 'selected' : '' }}>
                                        {{ $status->nombre }}
                                    </option>
                                @endforeach
                            </select>
            ...
            <div class="section-title">Integrantes</div>
                <div class="grid-2">
                    ...
                    <label>
                        ...
                        <button  type="button" onclick="abrirModal()" class="guardar"style="text-align: right;">Seleccionar </button>
                        <div id="previewParticipantes">
                            <ul class="listado" id="listaParticipantes">
                               @foreach($participanteSeleccionados as $p)
                                    <li data-id="{{ $p->idUsuario }}">
                                        {{ $p->usuario->nombres }} {{ $p->usuario->apellido_paterno }} {{ $p->usuario->apellido_materno }}
                                    </li>
                                @endforeach
                            </ul>
                    ...
                    <label>
                        ...
                        <button   type="button" onclick="abrirColaboradores()" class="guardar"
                            style="text-align: right;">Seleccionar</button>
                        <div id="previewColaboradores">
                            <ul class="listado" id="listaColaboradores">
                          @foreach($colaboradoresSeleccionados as $c)
                                <li data-id="{{ $c->idUsuario }}">
                ...
                <div class="section">
                    ...
                    <div class="grid-2">
                        <label>
                            Nombre del proyecto
                            <input type="text" name="Nombre" value="{{ $proyecto->descripcion->nombre ?? '' }}"
                                required>
                        </label>
                        <label>
                            Propuesta de valor
                            <input type="text" name="PropValor"
                                value="{{ $proyecto->descripcion->propuesta_valor ?? 'No asignado' }}" required>
                        </label>
                    </div>
                    ...
                    <div class="grid-2">
                        ...
                        <label>
                            Objetivos Generales
                            <textarea name="ObjsGrals">{{ $proyecto->descripcion->objetivos_generales ?? '' }}</textarea>
                        </label>
                        <label>
                            Objetivos Específicos
                            <textarea name="ObjsEspec">{{ $proyecto->descripcion->objetivos_especificos ?? '' }}</textarea>
                        </label>
                        <label>
                            Estado del arte
                            <textarea name="EdoArte">{{ $proyecto->descripcion->estado_arte ?? '' }}</textarea>
                        </label>
                        <label>
                            Fortalezas
                        ...
                        <label class="display-flex content">
                            ...
                            @if (!empty($proyecto->descripcion->pdf))
                                <a href="{{ asset('storage/' . $proyecto->descripcion->pdf) }}" target="_blank"
                        ...
                        <label>
                            ...
                            @if (isset($proyecto->descripcion->pdf_path))
                                <div style="margin-top:8px;">
                                    <a href="{{ asset('storage/' . $proyecto->descripcion->pdf_path) }}"
                                        target="_blank">Ver PDF actual</a>
                                </div>
                            @endif
                        </label>
                    </div>
